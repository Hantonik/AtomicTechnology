...
public class AtomicSmelterTile extends AbstractSmelterTile<AtomicSmelterRecipe> {
    public AtomicSmelterTile() {
        super(AtomicTiles.ATOMIC_SMELTER.get(), Configs.ATOMIC_SMELTER_RF_CAPACITY.get());
    }
    ...
    public void tick() {
        ...
        if (world != null) {
            ...
            if (this.recipe == null || !this.recipe.matches(recipeInventory, world) && this.progress == 0)
                this.recipe = world.getRecipeManager().getRecipe(AtomicRecipeTypes.ATOMIC_SMELTER, recipeInventory, world).orElse(null);
    ...
    public ITextComponent getDisplayName() {
        return Localizable.of("containers.atomictechnology.atomic_smelter").build();
    }

    @Nullable
    @Override
    public Container createMenu(int windowId, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        return AtomicSmelterContainer.create(windowId, playerInventory, this::isUsableByPlayer, this.inventory, this.data, this.getPos());
    }
}
