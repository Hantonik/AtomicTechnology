...
public class AdvancedSmelterTile extends AbstractSmelterTile<AdvancedSmelterRecipe> {
    public AdvancedSmelterTile() {
        super(AtomicTiles.ADVANCED_SMELTER.get(), Configs.ADVANCED_SMELTER_RF_CAPACITY.get());
    }
    ...
    public void tick() {
        ...
        if (world != null) {
            ...
            if (this.recipe == null || !this.recipe.matches(recipeInventory, world) && this.progress == 0)
                this.recipe = world.getRecipeManager().getRecipe(AtomicRecipeTypes.ADVANCED_SMELTER, recipeInventory, world).orElse(null);
    ...
    public ITextComponent getDisplayName() {
        return Localizable.of("containers.atomictechnology.advanced_smelter").build();
    }

    @Nullable
    @Override
    public Container createMenu(int windowId, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        return AdvancedSmelterContainer.create(windowId, playerInventory, this::isUsableByPlayer, this.inventory, this.data, this.getPos());
    }
}
