...
public class AdvancedSmelter extends BasicTileBlock {
    private static final DirectionProperty FACING = HorizontalBlock.HORIZONTAL_FACING;

    public AdvancedSmelter() {
        super(Properties.create(Material.IRON)
                .sound(SoundType.METAL)
                .setRequiresTool()
                .hardnessAndResistance(6.0F, 12.0F)
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new AdvancedSmelterTile();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult trace) {
        if (!world.isRemote) {
            ...
            if (tile instanceof AdvancedSmelterTile) {
    ...
    public void onReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (tile instanceof AdvancedSmelterTile) {
                AdvancedSmelterTile smelter = (AdvancedSmelterTile) tile;
