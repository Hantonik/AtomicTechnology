...
public class EliteSmelterCraftingCategory implements IRecipeCategory<ISmelterRecipe> {
    ...
    public EliteSmelterCraftingCategory(IGuiHelper guiHelper) {
        this.background = guiHelper.createDrawable(TEXTURE, 0, 0, 154, 78);

        this.icon = guiHelper.createDrawableIngredient(new ItemStack(AtomicBlocks.ELITE_SMELTER.get()));
    }
    ...
    public List<ITextComponent> getTooltipStrings(ISmelterRecipe recipe, double mouseX, double mouseY) {
        ...
        if (mouseX > 101 && mouseX < 121 && mouseY > 6 && mouseY < 14)
            return Collections.singletonList(Localizable.of("title." + AtomicTechnology.MOD_ID + ".elite_smelter").build());
