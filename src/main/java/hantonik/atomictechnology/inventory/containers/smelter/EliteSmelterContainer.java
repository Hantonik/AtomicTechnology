...
public class EliteSmelterContainer  extends Container {
    private final Function<PlayerEntity, Boolean> isUsableByPlayer;
    private final IIntArray data;
    private final BlockPos pos;

    private EliteSmelterContainer(ContainerType<?> type, int id, PlayerInventory playerInventory, PacketBuffer buffer) {
        this(type, id, playerInventory, p -> false, (new BasicSmelterTile()).getInventory(), new IntArray(5), buffer.readBlockPos());
    }

    private EliteSmelterContainer(ContainerType<?> type, int id, PlayerInventory playerInventory, Function<PlayerEntity, Boolean> isUsableByPlayer, BasicItemStackHandler inventory, IIntArray data, BlockPos pos) {
        super(type, id);
    ...
    public static EliteSmelterContainer create(int windowId, PlayerInventory playerInventory, PacketBuffer buffer) {
        return new EliteSmelterContainer(AtomicContainers.ELITE_SMELTER.get(), windowId, playerInventory, buffer);
    }

    public static EliteSmelterContainer create(int windowId, PlayerInventory playerInventory, Function<PlayerEntity, Boolean> isUsableByPlayer, BasicItemStackHandler inventory, IIntArray data, BlockPos pos) {
        return new EliteSmelterContainer(AtomicContainers.ELITE_SMELTER.get(), windowId, playerInventory, isUsableByPlayer, inventory, data, pos);
    }
