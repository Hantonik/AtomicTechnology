...
public class AdvancedSmelterContainer  extends Container {
    private final Function<PlayerEntity, Boolean> isUsableByPlayer;
    private final IIntArray data;
    private final BlockPos pos;

    private AdvancedSmelterContainer(ContainerType<?> type, int id, PlayerInventory playerInventory, PacketBuffer buffer) {
        this(type, id, playerInventory, p -> false, (new BasicSmelterTile()).getInventory(), new IntArray(5), buffer.readBlockPos());
    }

    private AdvancedSmelterContainer(ContainerType<?> type, int id, PlayerInventory playerInventory, Function<PlayerEntity, Boolean> isUsableByPlayer, BasicItemStackHandler inventory, IIntArray data, BlockPos pos) {
        super(type, id);
    ...
    public static AdvancedSmelterContainer create(int windowId, PlayerInventory playerInventory, PacketBuffer buffer) {
        return new AdvancedSmelterContainer(AtomicContainers.ADVANCED_SMELTER.get(), windowId, playerInventory, buffer);
    }

    public static AdvancedSmelterContainer create(int windowId, PlayerInventory playerInventory, Function<PlayerEntity, Boolean> isUsableByPlayer, BasicItemStackHandler inventory, IIntArray data, BlockPos pos) {
        return new AdvancedSmelterContainer(AtomicContainers.ADVANCED_SMELTER.get(), windowId, playerInventory, isUsableByPlayer, inventory, data, pos);
    }
